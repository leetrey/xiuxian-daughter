...
当前可填写已运行的物品、种植、加工、建筑、培养活动和剧情；剧情支持条件、三阶段表现、物品费用/奖励及活动、图纸解锁。战斗、比赛、继承等尚未实现的系统不能仅通过这里的表启用，参见 [开发进度](05-development-progress.md)。
...
| [cave.json](../data/cave.json) | 建筑、初始图纸、允许产物分类、可用配方 ID、御灵与景观 | 灵田、丹炉等七种建筑 |
| [cultivation.json](../data/cultivation.json) | 日程、自由活动、物品使用效果、初始库存与培养数值 | 服草减压、服丹减压、习剑等 |
| [story.json](../data/story.json) | 剧情条件、前置、三阶段表现、物品费用/奖励与活动、图纸解锁 | 一条突破主线与一条草药对话支线 |
...
检查覆盖 JSON 读取、物品分类、投入产出数量、初始库存及活动/建筑/配方的物品引用、建筑产物分类等。培养表还检查属性分组、结果倍率、压力档的结构、区间连续性及概率数组；漏填或错填时返回带字段位置的错误并以退出码 1 结束，不继续进入游戏。它不是经济平衡验证，也不会穷举所有未列出的字段或玩法条件。
...
| rewards | 可选奖励对象，只支持下面的 items、activities、blueprints、unlock_timing |
...
家园与未填 `portrait` 的对话都按实际纹理宽度三等分、高度取整图，因此可以替换不同分辨率的三列立绘。显式填写的 `region` 不自动缩放，替换该图后应同步更新像素坐标；只填 `texture` 时显示整图。
...
`items` 是物品 ID 到正整数数量，实际入库；`activities` 是 cultivation.json 中的活动 ID 数组。只有活动设了 `requires_unlock: true` 才会被剧情门槛锁住。`blueprints` 是 cave.json 中的建筑 ID 数组，表示对应建筑图纸，例子见第 8 节。`unlock_timing` 同时控制本节点的活动和图纸何时生效，默认 `next_turn`（下回合），也可填 `immediate`（对话完成后）；物品始终完成时入库。父亲恢复相关经济解锁尚未接入此处。

同一节点一局只完成一次；按阶段切换文本不会重复收费发奖。末句会显示物品变动与开放时机。多条剧情依次完成后重新检查剩余库存，不在开始时预扣整批；支线产生的新主线条件等下一个剧情检查点，不逆向重进主线。

图纸不收解锁材料，不要添加“图纸解锁费用”；`costs` 仅表示这个剧情事件本身明确需要消耗的普通物品。也不要用普通 costs 冒充父亲恢复或土地扩张的升级费：其继承减免还未实现。完整主线教学、剧情选项、任务日志、并发组与存档尚未接入。
...
每种建筑都需要对应图纸。图纸 ID 直接使用 `buildings` 中的建筑 ID，不在 items.json 中创建“图纸物品”，也没有张数或研究费用。获得一次后可反复建造，每栋仍支付该建筑的 `costs`，并检查空间与占地。

`cave.json` 的根字段 `initial_blueprints` 为必填数组，列出开局已掌握的建筑 ID；可填 `[]`，不能省略，不能含未知或重复 ID。当前样例保留全部七种图纸，方便既有试玩，不代表正式开局内容；这也不是已实现的继承预算界面。

例如让灵泉图纸通过剧情获得，先从初始列表删除 `spring`：

```json
"initial_blueprints": ["house", "herb_field", "grove", "quarry", "alchemy", "scarecrow"]
```

再在你编写的剧情节点上添加以下奖励；其他条件、台词仍按第 7 节填写。仅发图纸时省略 costs 或填 `{}`：
...
读完对话才登记图纸，下回合开始时可建造；普通奖励若需当场可用，改为 `immediate`。父亲恢复升级仍遵循下回合生效的既有规则。菜单能查看未获得/待生效的建筑，但建造按钮禁用，规则层也会拒绝绕过按钮的建造请求。

重复图纸不会扣材料、转换成物品或再次增加一份资格；已掌握图纸不等于完成相应故事，也不扩展土地。新增建筑时，除定义 `buildings`，还应按你的内容安排把 ID 放进初始列表或剧情奖励，否则保持未获得状态。

### 外观图格

在 `cave.json` 的某项建筑定义中加入 `"sprite_index": 1`，即可指定使用建筑图集的第 1 格；不改变建筑分类、工作类型或生产规则。字段可省略以兼容旧配置，但新内容建议显式填写。

| 序号 | 样例造型 |
|---|---|
| 0 / 1 / 2 / 3 | 竹舍 / 灵田 / 林场 / 采石场 |
| 4 / 5 / 6 / 7 | 丹炉 / 稻草人 / 灵泉 / 入口 |

序号从 0 开始，从左到右、从上到下排列；目前只接受整数 0–7，字符串、小数和越界值会报错。省略时沿用同类映射，未知类型使用 5 号图格。图集仍约定为 4 列×2 行，但单格像素尺寸根据实际图片计算，不写死为 384×512。新增独立美术或调整排列时见 [素材替换说明](../assets/README.md)。
